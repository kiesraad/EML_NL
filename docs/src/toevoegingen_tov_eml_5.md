...
De elementen `PreviousValidVotes`, `PreviousCast`, `PreviousTotalCounted`, `PreviousRejectedVotes` en `PreviousUncountedVotes` zijn elementen die optioneel in een 510a, 510b of 510c voor kunnen komen in het geval dat element `Phase Phasecode="corrigendum"` heeft. De bedoeling van deze elementen is om het vorige aantal (indien er één corrigendum is, is dit het aantal van de eerste zitting. Indien dit corrigendum nummer `i` is, dan betreft dit de aantallen van corrigendum `i-1`) vast te leggen zodat deze vergeleken kan worden met het gecorrigeerde aantal (hetzelfde element maar dan zonder prefix ‘Previous’). Dit element komt alleen voor <u>indien het aantal in het corrigendum aangepast is</u>. Als de waarde in het corrigendum niet aangepast is, komt het element dus ook niet voor.
