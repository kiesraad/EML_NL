...
Een voorbeeld van de EML_NL 520 voor een Tweede Kamerverkiezing is hieronder afgebeeld. Ook hier geldt dat `ShortCodes` worden gebruikt zodat de kandidaten gekoppeld kunnen worden aan de kandidatenlijsten. Het `CandidateIdentifier` `Id` is hier de volgorde waarin de kandidaten gekozen zijn *en dus niet het nummer op de kandidatenlijst*:
